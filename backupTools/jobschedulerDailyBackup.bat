...
set BACKUPSCRIPT=C:\FGA_SOFT_INTEGRATION\backupTools\jobscheduler_sqlserverBackup.sql 
...
sqlcmd.exe -S FX026630M\SQLEXPRESS -d jobscheduler -i %BACKUPSCRIPT% -o "%BACKUP_DIR%\%YYYYMMDD%_messages.txt" -X

xcopy "%BACKUP_DIR%" "%DESTINATION%\%YYYYMMDD%_%MYTIME%" /D/E

del "%BACKUP_DIR%\*jobscheduler.bak"
