...
order by constraint_name, referenced_table_name, keyno";
	...
	function _query($sql,$inputarr=false)
	{
		...
		if (!is_array($sql) && preg_match('/^\\s*insert/i', $sql)) {
			...
			/* debut modif
			echo "<br/>library/adodb/drivers/adodb-odbc_mssql.inc.php for debugging database only (line 477):<br/>SQL-Error: " . $sql . "  --- " . var_export($inputarr, true);
			$sql2 = $sql;
			foreach($inputarr as $a => $b)	{
				$p = strpos($sql2, "?");
				if(gettype($b) == "string") $b = "'" . $b . "'";
				$sql2 = substr($sql2, 0, $p) . " " . $b . substr($sql2, $p + 1);
			}
			echo "<br/><br/>" . $sql2;
			// fin modif
*/
