...
class SourceRepo {
	...
	function stats( $p_all=true ) {
		...
		if ( $p_all ) {
		// patch GC : ansi sql
			$t_query = "SELECT COUNT(DISTINCT 1) FROM $t_file_table AS f
...
class SourceChangeset {
	...
	function save() {
		...
		if ( 0 == $this->id ) { # create
			...
			// Patch GC : support des types MS SQL Datetime 2005
			db_query_bound( $t_query, array(
				$this->repo_id, $this->revision, $this->parent, $this->branch,
				$this->user_id, new DateTime($this->timestamp) , $this->author, $this->message, $this->info,
			...
			db_query_bound( $t_query, array(
				...
				new DateTime($this->timestamp), $this->author, $this->message,
	...
	function save_bugs( $p_user_id=null ) {
		...
		if ( count( $t_bugs_added ) > 0 ) {
			...
			// Patch GC: forcer le type
			foreach( $t_bugs_added as $t_bug_id ) {
				$t_query .= ( $t_count == 0 ? '' : ', ' ) .
					'(' . db_param() . ', ' . db_param() . ')';
				$t_params[] = (int)$this->id;
				$t_params[] = (int)$t_bug_id;
				$t_count++;
			}
