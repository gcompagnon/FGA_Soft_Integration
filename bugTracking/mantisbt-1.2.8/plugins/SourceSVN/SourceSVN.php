				...
				if ( strlen( $t_line ) == 0 ) {
					...
					if ( preg_match( '/^\s+([a-zA-Z])\s+([^\(]+)(?: \(from [^\)]+\))?/', $t_line, $t_matches ) ) {
						...
						if ( is_blank( $t_changeset->branch) ) {
							...
							// Patch GC : pour supporter : aucune arborescense pour la branche principale /trunk:
							// chercher les branches puis les versions tags en premier
								...
								# Fall back to just using the root folder as the branch name
									...
									$t_changeset->branch = $t_matches[1];
								# patch GC: si il n y a pas de répertoire : mettre "Main"
								} else {
									$t_changeset->branch = 'Main';
								}
